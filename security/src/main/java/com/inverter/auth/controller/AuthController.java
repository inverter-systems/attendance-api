...
public class AuthController {
	...
	@PostMapping("/auth")
	public ResponseEntity<Response<LoginDTO>> auth(@Valid @RequestBody LoginDTO login, HttpServletResponse resp,
		...
		try {
			...
			this.userRepo.findByUsername(login.username())
					.orElseThrow(() -> new SecurityException(msg.get("user.auth.user.error.not.exists")));
			...
			response.setData(LoginDTO.buildLoginDTO(user));
	...
	public ResponseEntity<Response<String>> logout(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    try {
	        // Adiciona o cookie expirado na resposta
	        response.addHeader(HttpHeaders.SET_COOKIE, getEmptyCookie());
	        
	        resp.setData("Logout realizado com sucesso");
	        return ResponseEntity.ok(resp);
	        
	    } catch (Exception e) {
	        resp.getErrors().add("Erro durante o logout: " + e.getMessage());
	...
	public ResponseEntity<Response<LoginDTO>> validate(HttpServletRequest req) {
		var response = new Response<LoginDTO>();
		try {
			var user = tokenService.authenticateRequest(req);
			response.setData(LoginDTO.buildLoginDTO(user));
		} catch (Exception e) {
			response.getErrors().add(e.getMessage());
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
	...
	private String getCookie(String token) {
		...
		ResponseCookie cookie = ResponseCookie.from("access_token", token).httpOnly(true).secure(true).path("/")
				.sameSite("Strict").maxAge(expiresAt.toMinutes()).build();

		return cookie.toString();
	}
	
	private String getEmptyCookie() {
		ResponseCookie cookie = ResponseCookie.from("access_token", "").httpOnly(true).secure(true).path("/")
